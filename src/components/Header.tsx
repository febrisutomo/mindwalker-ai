...
export default function Header({ dictionary, lang }: { dictionary: Dictionary; lang: Locale }) {
  ...
  return (
    <>
      ...
      >
        <div className={`absolute inset-0 transition-opacity duration-300 ${isScrolled ? 'bg-white/80 dark:bg-[#101622]/80 backdrop-blur-md shadow-sm' : 'bg-transparent'
          }`}></div>
        <div className="w-full max-w-[1280px] mx-auto px-6 sm:px-10 lg:px-20 flex items-center justify-between whitespace-nowrap relative z-10">
          ...
          <div className="hidden md:flex flex-1 justify-end gap-8">
            <div className="flex items-center gap-9">
              <Link
                className={`text-sm font-medium leading-normal transition-colors cursor-pointer ${isScrolled ? 'text-[#111318] hover:text-primary' : 'text-gray-200 hover:text-white'
                  }`}
                href={`/${lang}#home`}
              >
              ...
              <div className="relative group">
                <button
                  className={`flex items-center gap-1 text-sm font-medium leading-normal transition-colors cursor-pointer ${isScrolled ? 'text-[#111318] hover:text-primary' : 'text-gray-200 hover:text-white'
                    }`}
                >
              ...
              <div className={`flex items-center rounded-full p-1 border transition-colors ${isScrolled ? 'bg-gray-100 border-gray-200' : 'bg-white/10 border-white/20'
                }`}>
                <Link
                  href="/id"
                  className={`px-3 py-1 rounded-full text-xs font-medium transition-all duration-300 ${lang === 'id'
                ...
                <Link
                  href="/en"
                  className={`px-3 py-1 rounded-full text-xs font-medium transition-all duration-300 ${lang === 'en'
                    ? 'bg-primary text-white font-bold'
                    : isScrolled ? 'text-[#111318] hover:bg-gray-200' : 'text-white hover:bg-white/10'
                    }`}
                >
      ...
      < div
        id="mobileMenu"
        className={`mobile-menu md:hidden fixed inset-0 bg-white dark:bg-[#101622] z-[60] flex flex-col items-center justify-center transition-all duration-300 ${mobileMenuOpen ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible -translate-y-full'
          }`
        }
      >
        <button
        ...
        <nav className="px-6 py-4 w-full max-w-sm text-center font-medium">
          <div className="flex flex-col space-y-4 w-full cursor-pointer">
            ...
            <div className="flex justify-center gap-4 mt-6">
              <Link
                href="/id"
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 border ${lang === 'id' ? 'bg-primary text-white border-primary' : 'text-[#111318] dark:text-white border-gray-300 dark:border-gray-700'
                  }`}
              >
                Bahasa Indonesia
              </Link>
              <Link
                href="/en"
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 border ${lang === 'en' ? 'bg-primary text-white border-primary' : 'text-[#111318] dark:text-white border-gray-300 dark:border-gray-700'
                  }`}
              >
                English
              </Link>
